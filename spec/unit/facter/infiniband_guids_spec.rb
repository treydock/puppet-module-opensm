...
describe 'infiniband_guids fact' do
  ...
  it "should return a single GUID" do
    Facter::Core::Execution.expects(:exec).with("/usr/sbin/ibstat -p 2>/dev/null").returns("0x0000000000000001")
  ...
  it "should not be present if ibstat -p returns nil" do
    Facter::Core::Execution.expects(:exec).with("/usr/sbin/ibstat -p 2>/dev/null").returns(nil)
    expect(Facter.fact(:infiniband_guids).value).to be nil
  end

  it "should not be present if ibstat -p returns an empty string" do
    Facter::Core::Execution.expects(:exec).with("/usr/sbin/ibstat -p 2>/dev/null").returns('')
    expect(Facter.fact(:infiniband_guids).value).to be nil
  end
end
